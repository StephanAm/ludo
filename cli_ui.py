...
from typing import IO, Any, Callable, Dict, List, Optional, Sequence, Tuple, Union,Iterable
...
def read_input(msg:str=None) -> str:
...
def ask_string(*question: Token, default: Optional[str] = None, inline:bool=True) -> Optional[str]:
...
def ask_choice(
    *prompt: Token,
    ...
    :param choices: an Iterable of arbitrary elements
    ...
    :param default: a default value that should be returned when the prompt is empty.
    ...
    default_desc = func_desc(default) if default else None
    for i, choice in enumerate(choices, start=1):
        choice_desc = func_desc(choice)
        tokens = ["  ", blue, "%i" % i, reset, choice_desc]
        if default_desc == choice_desc:
            tokens.append('<')
        info(*tokens)
    keep_asking = True
    res = None
